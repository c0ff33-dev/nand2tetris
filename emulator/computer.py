"""
Pygame-based computer emulator for Nand2Tetris.
...
Usage:
    python emulator/computer.py <file.asm> [--scale N] [--fps N]
"""
...
from accelerated_engine import ACCEL_AVAILABLE, AcceleratedEngine
from engine import Engine

# Standard computer memory map
SCREEN_BASE = 16384
SCREEN_END = 24576
KBD_ADDR = 24576
...
def main() -> None:
    """Entry point for the computer emulator."""
    import argparse

    parser = argparse.ArgumentParser(description="Nand2Tetris computer emulator")
    ...
    if ACCEL_AVAILABLE:
        ...
        print("Computer Emulator: Using compiled backend")
    else:
        engine = Engine()
        print(
            "Computer Emulator: Compiled backend unavailable (build with `python interpreter/build_accelerator.py`)"
        )
    engine.load(args.file)
    print("Computer Emulator: Loaded %s (%d instructions)" % (args.file, len(engine.rom_raw)))

    pygame.init()
    window_w = SCREEN_WIDTH * args.scale
    window_h = SCREEN_HEIGHT * args.scale
    window = pygame.display.set_mode((window_w, window_h))
    pygame.display.set_caption("Computer Emulator - %s" % args.file)
    ...
    while running:
        ...
        pygame.display.set_caption("Computer - %s | %.1f FPS | %.2f MHz" % (args.file, fps, effective_mhz))
        pygame.display.flip()

        clock.tick(args.fps)

    pygame.quit()
    print("Computer Emulator: %d total cycles executed" % total_cycles)
