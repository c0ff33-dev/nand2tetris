...
**Goal:** Use a glob pattern (`*.vm`) to discover files dynamically, with logic to ensure `Sys.vm` is always first (spec requirement), `Main.vm` second (personal convention) and `*_out.vm` files are excluded.  
...
## 4. Interpreter: Use symlinks for OS libraries
...
## 5. Interpreter: Add breakpoints to CLI args
...
## 6. Interpreter: Implement stack display in debug TUI
...
## 7. Interpreter: Separate interpreter from test harness
